...
import { Users, FileText, Crown, CreditCard, Share2, HardDrive, TrendingUp, UserPlus } from 'lucide-react';
...
export default function AdminAnalyticsPage() {
  ...
  const cards = [
    ...
    { label: 'Premium Users', value: stats.premium_users, icon: Crown, color: 'bg-amber-50 text-amber-600', gradient: 'from-amber-500/20' },
    ...
    { label: 'Total Revenue', value: `Rs. ${stats.total_revenue}`, icon: CreditCard, color: 'bg-emerald-50 text-emerald-600', gradient: 'from-emerald-500/20' },
    { label: 'Active Subs', value: stats.active_subscriptions, icon: Crown, color: 'bg-yellow-50 text-yellow-600', gradient: 'from-yellow-500/20' },
    { label: 'New Users (30d)', value: stats.recent_signups, icon: UserPlus, color: 'bg-cyan-50 text-cyan-600', gradient: 'from-cyan-500/20' },
    { label: 'Revenue (30d)', value: `Rs. ${stats.recent_payments}`, icon: TrendingUp, color: 'bg-rose-50 text-rose-600', gradient: 'from-rose-500/20' },
  ];

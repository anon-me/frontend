...
import { Search, Crown, Shield, Ban, Trash2, Eye } from 'lucide-react';
...
export default function AdminUsersPage() {
  ...
  const handleTogglePremium = async (user: any) => {
    try {
      await adminApi.updateUser(user.id, { is_premium: !user.is_premium });
                    ...
                    <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-slate-800 flex items-center gap-1.5">{u.name} {u.is_premium && <Crown size={14} className="text-amber-500 fill-amber-500/20" />}</p>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleTogglePremium(u)} title="Toggle premium"
                            className={`p-2 rounded-xl transition-all duration-300 ${u.is_premium ? 'text-amber-600 bg-amber-50 hover:bg-amber-100' : 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50'}`}>
